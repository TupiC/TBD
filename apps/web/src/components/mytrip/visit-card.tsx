...
export default function VisitCard({
    ...
    const label =
        ariaLabel ?? `${place.name} — ${categoryLabel[place.category]}`;
    ...
    const summaryCls =
        size === "compact"
            ? "mt-1 line-clamp-2 text-sm/6"
            : "mt-2 line-clamp-3 text-base/6";
        ...
        >
            ...
            <div className="absolute inset-0">
                <Image
                    ...
                    unoptimized
                />
                <div className="absolute inset-0 bg-linear-to-br from-black/70 via-black/40 to-black/30" />
            </div>

            {/* CONTENT */}
            <div className="relative p-4 md:p-6 text-white">
                <div className="backdrop-blur-[2px] rounded-xl">
                    ...
                    <div className="flex justify-between items-start gap-3">
                        ...
                        {onDismiss && (
                            ...
                            >
                                ...
                                <X
                                    className="opacity-90 group-hover:opacity-100 w-5 h-5"
                                    aria-hidden
                                />
                            </button>
                        )}
                    ...
                    {/* Summary */}
                    <p className={cx(summaryCls, "text-white/90")}>
                        {place.summary}
                    </p>

                    {/* Meta row */}
                    <div className="flex flex-wrap items-center gap-2 md:gap-3 mt-3 md:mt-4">
                        <span className="inline-flex items-center bg-white/15 backdrop-blur px-3 py-1 rounded-full font-medium text-sm">
                            {categoryLabel[place.category] ?? "Experience"}
                        </span>

                        {/* Duration */}
                        <span className="inline-flex items-center gap-1 bg-white/10 px-3 py-1 rounded-full text-white/90 text-sm">
                            <Clock className="opacity-90 w-4 h-4" aria-hidden />
                            {durationLabel}
                        </span>
                        ...
                        <span className="inline-flex items-center bg-white/10 px-3 py-1 rounded-full text-white/90 text-sm">
                            {seasonText(
                                place.opened_months.start_month,
                                place.opened_months.end_month
                            )}
                        </span>

                        {place.free_access && (
                            <span className="inline-flex items-center bg-emerald-400/20 px-3 py-1 rounded-full text-emerald-100 text-sm">
                                Free access
                            </span>
                        )}
                    </div>
                    ...
                    {/* Footer actions */}
                    <div className="flex justify-between items-center gap-3 mt-4 md:mt-6">
