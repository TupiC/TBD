...
const Page = (): React.JSX.Element => {
    ...
    const { setDate, setDays, setType } = useOnboardingStore();
    const { date, days, type } = useOnboardingStore();
    ...
    const steps: JSX.Element[] = [
        ...
        <ContentPage key="content2" back={back} next={() => setSubmitted(true)}>
            <div className={`flex flex-col gap-1`}>
                ...
                <Heading type="h3">
                    Jede Person ist unterschiedlich, was hast du vor in
                    Salzburg?
                </Heading>
            </div>
            <div className="flex flex-col gap-4">
        ...
        return (
            <div>
                <main className={`${loading ? "invisible" : ""}`}>
                    <div className="flex flex-col justify-center items-center gap-5 w-screen h-dvh text-center">
                        <AnimationIcon
                            duration={0.35}
                            loop
                            className="w-10 h-10"
    ...
    if (currentStepIndex === 0) {
        return (
            <div
                className={`${loading ? "h-dvh w-screen overflow-hidden" : ""}`}
            >
                {loading && (
